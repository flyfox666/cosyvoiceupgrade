...
# 设置日志级别 - 必须在导入其他模块之前完成
import os
os.environ['NUMBA_DISABLE_JIT'] = '0'  # 保持 JIT 但禁用调试
os.environ['NUMBA_WARNINGS'] = '0'     # 禁用 Numba 警告
os.environ['NUMBA_LOGGING_LEVEL'] = 'ERROR'  # Numba 只显示错误

logging.basicConfig(level=logging.INFO, format='%(message)s')  # 简洁格式
...
def prompt_wav_recognition(prompt_wav):
    ...
    try:
        # 使用 Whisper 进行识别
        result = asr_model.transcribe(prompt_wav, language=None, task="transcribe")
...
def save_voice_to_library(voice_name, audio_file, prompt_text):
    """保存音色到音色库"""
    if not voice_name:
        gr.Warning("请输入音色名称")
...
def main():
    with gr.Blocks() as demo:
        ...
        save_voice_btn.click(
            ...
            inputs=[save_voice_name, prompt_wav_upload, prompt_text], 
...
if __name__ == '__main__':
    ...
    # 加载 ASR 模型（使用 Whisper）
    asr_model = None
    try:
        print("\n🎬 加载 Whisper ASR 模型 (base 版本)...")
        asr_model = whisper.load_model("base", device="cpu")
        print("✅ Whisper ASR 模型加载成功（支持中英文等多语言识别）")
    except Exception as e:
        print(f"⚠️ Whisper 加载失败: {e}")
        print("   音频识别功能将不可用，但不影响 TTS 功能")
        asr_model = None
